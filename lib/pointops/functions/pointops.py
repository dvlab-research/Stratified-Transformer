...
class KNNQueryMM(Function):
    @staticmethod
    def forward(ctx, nsample: int, xyz1: torch.Tensor, xyz2: torch.Tensor = None) -> Tuple[torch.Tensor]:
        """
        KNN Indexing
        input: nsample: int32, Number of neighbor
    ...
    def forward(self, xyz: torch.Tensor, new_xyz: torch.Tensor = None, features: torch.Tensor = None, idx: torch.Tensor = None) -> torch.Tensor:
        ...
        if idx is None:
            if self.radius is not None:
                ...
                #idx = knnquery_mm(self.nsample, xyz, new_xyz)   # (b, m, nsample)
